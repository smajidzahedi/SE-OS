...
\newlength{\slidewidth}
\setlength{\slidewidth}{0.8\textwidth}

\title{Lecture 24 --- Caching }
...
\begin{center}
	\includegraphics[width=0.4\slidewidth]{images/cacherules.jpg}
\end{center}
...
Caches typically operate on blocks of a given size.
...
The address will be mapped to a line; ideally that line is in the cache.

If the requested line is, in fact,  in the cache, we call that a cache \alert{hit}.
...
If the line is not found in the cache, it is considered a cache \alert{miss}.

In case of a miss, we must load the line from memory, a slow operation.
...
The percentage of the time  a line is found in the cache is called the \alert{hit ratio}.

\begin{center}
	\includegraphics[width=0.4\slidewidth]{images/hitratio.jpg}
\end{center}
...
\quad $t_{c}$ is the time required to load a line from cache.\\
\quad $t_{m}$ is the time to load a line from memory.
...
\begin{center}
\includegraphics[width=0.9\slidewidth]{images/caches.png}
\end{center}
...
If the L2 cache contains the desired line, it will be copied to the L1 cache and sent to the CPU.
...
Main memory (RAM) acts as a cache for disk (secondary storage)

Cannot fit all pages of all processes in memory simultaneously

Only a subset of pages is kept in memory at any time

OS must decide which pages to keep in memory and which pages to evict to disk

When a process accesses a page that is not currently in memory, a \alert{page fault} occurs

OS loads page on demand and may evict another (replacement policy)
\end{frame}

\begin{frame}
\frametitle{Page Replacement Algorithms}

Whenever a page fault occurs, the operating system needs to choose which page to \alert{evict} from the memory to make space for the new one.

This assumes that the memory is full, which it likely is except at system startup.
...
If a page has been altered in memory, then that change has to be written to main memory at some point.

It can be done immediately when the page is changed, or it can be done when the page is evicted from the memory.
...
If a page has not been modified in memory, it can simply be overwritten.
...
\begin{center}
	\includegraphics[width=\slidewidth]{images/palantir.jpg}
\end{center}
...
\begin{center}
	\includegraphics[width=0.6\slidewidth]{images/cannotlose.jpg}
\end{center}
...
If a page is referenced often, we want to keep it in memory, not evict it just because it has been in the memory the longest.
...
\begin{center}
	\includegraphics[width=0.5\slidewidth]{images/secondbreakfast.jpg}
\end{center}
...
In some textbooks this is called the Clock replacement algorithm because we can think of the memory as a circular buffer.

\begin{center}
	\includegraphics[width=0.3\slidewidth]{images/mathclock.jpg}
\end{center}
...
When a page in the memory is accessed, move that page to the back of the list.
...
\begin{center}
\includegraphics[width=\slidewidth]{images/lru-linkedlist.png}
\end{center}
...
Thus far all strategies for putting things in the memory have been ``on-demand''.
...
Technically, no pages of the process need to be loaded into the memory to start with; we can simply suffer through a whole bunch of page faults.

This is rather slow; it might be better to load multiple pages into the memory.
...
So we may have multiple processes with data in the cache at a given time.
...
If process $P_{1}$ has a page fault, do we replace the least recently used page in all the memory (global replacement)?

Or do we replace the least recently used page in the memory that belongs to $P_{1}$ (local replacement)?
...
Local algorithms give each process some fixed number of pages in the memory.

Global algorithms dynamically allocate memory space to different programs based on their needs; the number of pages in the memory for each process varies.

Global algorithms allegedly work better, when processes' memory needs change over time.

If the local algorithm is used, we may be wasting some of the space in the memory for a process that does not really need it right now.
...
Suppose we have a large memory, and a dynamic allocation (global algorithm).

We want some intelligence to keep any process from having too many or too few pages in the memory.

To manage the complexity of how much of the memory should be allocated to a particular process, we might wish to keep track of the number of page faults.
...
PFF too high? More memory space is needed for that process.

PFF low? The process has too much memory space allocated.

Note: relies on assumption that more memory space means fewer page faults.\\
