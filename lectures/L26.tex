...
\lecture{ 26 --- Virtual Memory II }{\term}{Prepared by Jeff Zarnett, taught by Seyed Majid Zahedi}
...
The absolute minimum number of frames per process is determined by the instruction set architecture and equals the maximum number of distinct pages that a single instruction may need simultaneously. For example, on IBM 370, an SS‑type MOVE instruction (\texttt{MVC}) is 6 bytes long and may straddle two pages; it also has two memory operands that may each cross a page boundary. In the worst case, this single instruction can touch 2 pages for the instruction, 2 for the source operand, and 2 for the destination operand, for a total of 6 pages. Therefore, on such a machine a process must be allocated at least 6 frames.
