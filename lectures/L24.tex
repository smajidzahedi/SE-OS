...
\lecture{ 24 --- Caching }{\term}{Prepared by Jeff Zarnett, taught by Seyed Majid Zahedi}
...
Caches operate on blocks of a given size. An entry in a cache is often called a \textit{line}. As discussed, the CPU generates a memory address for a read or write operation. The address will be mapped to a line. Ideally, the line is found in the cache, because that would be faster. If the requested line is, in fact,  in the cache, we call that a cache \textit{hit}. If the line is not found in the cache, it is considered a cache \textit{miss}. In case of a miss, we must load the line from memory, a comparatively slow operation. The percentage of the time that a line is found in the cache is called the \textit{hit ratio}, because it is how often we have a cache hit. We can calculate the effective access time if we have a good estimate of the hit ratio (which is not overly difficult to obtain) and some measurements of how long it takes to load data from the cache and how long from memory. The effective access time is therefore computed as:

\begin{center}
Effective Access Time = $h \times t_{c} + (1-h) \times t_{m}$
\end{center}

Where $h$ is the hit ratio, $t_{c}$ is the time required to load a line from cache, and $t_{m}$ is the time to load a line from memory. Of course, we would like the hit ratio to be as high as possible.
...
If we have a miss in the L1 cache, the L2 cache is checked. If the L2 cache contains the desired line, it will be copied to the L1 cache and sent to the CPU. If it is not in L2, then L3 is checked. If it is not there either, it is in main memory and will be retrieved from there and copied to the in-between levels on its way to the CPU. Because caches have limited size, we have to manage this storage carefully.

\subsection*{Paging}
Modern operating systems use paging to give each process the illusion of a large, contiguous address space, even though physical memory is limited. A useful way to think about this is that main memory acts as a cache for data stored on disk (or other secondary storage). Just like an L1 or L2 cache cannot hold all data from main memory, RAM cannot hold all pages from all running processes. In a typical system, the total virtual address space across processes is far larger than physical memory, so only a subset of pages can reside in memory at any given time.

This raises a fundamental question: which pages should be kept in memory, and which should remain on disk? The operating system must make dynamic decisions about which pages to load (on demand) and which to evict when memory is full. In this analogy, each page frame in main memory corresponds to a cache line, and the OS effectively implements a page replacement policy (e.g., LRU-like approximations) at the page level. When a process accesses a page that is not currently in memory, a \emph{page fault} occurs, triggering the OS to bring that page into a frame—possibly evicting another page to make space. This interplay between memory and storage is the core of virtual memory management.

\subsection*{Page Replacement Algorithms}
Whenever a page fault occurs, the operating system needs to choose which page to \textit{evict} from (kick out of) the memory to make space for the new one. This assumes that the memory is full, which it likely is except at system startup. We could, of course, just select a random page, but we should do this task more intelligently, if we can.
...
If a page has been altered in memory, then that change has to be written to main memory at some point. It can be done immediately when the page is changed, or it can be done when the page is evicted from the memory. The second option means fewer main memory accesses, if a page is written to multiple times before it is sent to main memory. If that memory is shared, however, between multiple processors or an I/O device, then the delay in updating main memory may be intolerable. If a page has not been modified in memory, it can simply be overwritten. If all other factors are equal, we should replace a page that has not been modified, as the work to write it out to memory need not be done.
...
Thus far all of our strategies for putting things in the memory have been ``on-demand'': it is only when a page is needed and not found in the memory that a page is loaded into the memory. Suppose instead that the operating system takes some steps to guess about what pages might be needed next, based on the principle of temporal or spatial locality. This might reduce the amount of time spent waiting for a page in the future.
...
A situation in which pre-paging might be useful is when a program is started or swapped into memory. Technically, no pages of the process need to be loaded into the memory to start with; we can simply suffer through a whole bunch of page faults. This is rather slow; it might be better to load multiple pages into the memory to start with, though this will of course require some guesses about what pages will be needed~\cite{mos}.
...
Local algorithms give each process some (roughly) fixed number of pages in the cache. Global algorithms dynamically allocate cache space to different programs based on their needs, so the number of pages in the memory for each process can vary over time.

According to~\cite{mos}, global algorithms work better, especially when processes' memory needs change over time. If the local algorithm is used, we may be wasting some of the space in the memory for a process that does not really need it.

Suppose we have a sufficiently large memory, and a dynamic allocation (global algorithm) with some intelligence to keep any process from having too many or too few pages in the memory. To manage the complexity of how much of the memory should be allocated to a particular process, we might wish to keep track of the number of page faults with a measure called the \textit{page fault frequency} or PFF. In simplest terms, the PFF is the guide for telling whether a given process has too few, too many, approximately the right number of pages.

If the PFF is above some upper threshold, that indicates that more memory space is needed for that process. If the PFF is below some lower threshold, it suggests that the process has too much memory space allocated and could stand to part with some.
